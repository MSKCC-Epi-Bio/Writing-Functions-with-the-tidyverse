...
* Is there a better way to do this? Why don't `any_of()`/`all_of()` work
with `group_by`?
...
* `rlang::eval_bare()`: Evaluates the expression
...
* `across()`: if you want the user to provide a set of data-variables that are
then transformed
